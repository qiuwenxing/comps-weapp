    ...
    value: {
      ...
      observer(array) {
        if (array && array.length == 2) {
          if (this.data.regionList.length > 0) this.setValue(array);
          else initValue = array;
        }
      },
    },
    containsAll: {
  ...
  methods: {
    init() {
      ...
      if (regionList.length > 0) {
        regionData = new Map();
        if (containsAll) {
          regionData.set(headItem[0], { name: headItem[0], children: [] });
        }
    ...
    confirm() {
      ...
      this.triggerEvent("change", { values: values, names: curValue });
    },
    /**
     * 设置值
     * @param {string[]} array
     ...
     */
    hide() {
      let { isAnimation, origin, curValue, curIndex, citys, display } =
        this.data;
      //显示状态才执行隐藏
      if (display && !isAnimation) {
        ...
        let animationDataBody = animationBody
          .translateY("100%")
          .step()
          .export();
        this.setData({
    ...
    refreshCity(isReset = true) {
      ...
      if (curValue.length > 0) {
        ...
        if (containsAll && citys) citys.unshift(headItem[1]);
        if (citys.length > 0 && isReset) curValue[1] = citys[0];

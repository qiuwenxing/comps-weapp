...
Component({
  ...
  properties: {
    ...
    value: {
      ...
      observer(val) {
        console.log(val);
        if (val && val.length === 4) {
          this.setValue(val[0], val[1], val[2], val[3]);
        }
      },
    },
  },
  ...
  methods: {
    ...
    validate(result) {
      ...
      let isError = false;
      if (start.getTime() > end.getTime()) isError = true;
    ...
    setValue(startYear, startMonth, endYear, endMonth) {
  ...
  ready() {
    ...
    let { minYear, maxYear, years, months, value } = this.data;
    //初始化年份
    ...
    if (!value || value.length != 4) {
      this.setValue(year, month, year, month);
    }

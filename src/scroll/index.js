...
Component({
  ...
  properties: {
    ...
    backTopImage: {
      type: String,
      value: "../assets/images/top-scroll.png",
    },
  },
  data: {
    /**
  ...
  observers: {
    "backTopVisible,scroll": function () {
      const { backTopVisible, scroll } = this.data;
      this.setData({ backTopShow: backTopVisible && scroll > 300 });
    },
  },
  methods: {
    onScroll(e) {
    ...
    startHandle(e) {
      if (
        !this.data.loading &&
        (this.data.open == "down" || this.data.open == "all")
      ) {
        ...
        if (open == "down" || open == "all") {
          ...
          if (touchScrollTop <= 0 && direction == "down" && !isLockUp) {
            ...
            if (absMoveY <= distance) {
              ...
              offsetY =
                distance + distance * 0.5 + (absMoveY - distance * 2) * 0.2;
            }
            ...
            if (absMoveY > loadingHeight) {
              ...
              arrowAngle =
                ((arrowY > arrowHeight ? arrowHeight : arrowY) / arrowHeight) *
                180;
            }
            arrowAnimationData = arrowAnimation
              .rotateZ(arrowAngle)
              .step()
              .export();
          } else {
    ...
    pullUp() {
      if (
        !this.data.loading &&
        (this.data.open == "up" || this.data.open == "all")
      ) {
    ...
    onBackTop() {
      this.setData({ intoView: "scrollTop" });
    },
  },
  created() {
